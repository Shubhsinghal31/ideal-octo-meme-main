    ...
    (set, get) => ({
      ...
      createSession: async (teacherId, subject, section, course) => {
        try {
          ...
          const response = await axios.post('https://ideal-octo-meme-main.onrender.com/api/start-session', {
            subject,
            section,
            course
          });
      ...
      generateOtp: async (sessionId: string) => {
        try {
          const response = await axios.post('https://ideal-octo-meme-main.onrender.com/api/generate-otp', {
            sessionId
          });
          //@ts-ignore
      ...
      fetchAttendanceRecords: async (sessionId) => {
        try {
          const response = await axios.get(`https://ideal-octo-meme-main.onrender.com/api/session/${sessionId}/attendance`);
